...
function getListItem(index, place) {
  var el = document.createElement('li'),
      ...
      restaurantAddress = place.road_address_name || place.address_name || "주소 정보 없음",
      phone = place.phone || "전화번호 없음";

  var markerStr = `<span class="markerbg marker_${index + 1}">${index + 1}</span>`;
  var infoStr = `
        <div class="info">
            <h5 class="restaurant-name" data-name="${restaurantName}" data-address="${restaurantAddress}">${restaurantName}</h5>
  ...
  const restaurantElement = el.querySelector(".restaurant-name");
  if (restaurantElement) {
    restaurantElement.addEventListener("click", function() {
      console.log("가게 클릭됨:", restaurantName, restaurantAddress); // 디버깅 로그 추가
      showReviews(restaurantName, restaurantAddress);
    });
  } else {
    console.warn("restaurant-name 클래스를 찾을 수 없음!");
  }
...
function showReviews(restaurantName, restaurantAddress) {
  console.log("showReviews 실행됨!", restaurantName, restaurantAddress); // 디버깅 로그 추가
  const reviewPageUrl = `review.html?name=${encodeURIComponent(restaurantName)}&address=${encodeURIComponent(restaurantAddress)}`;
  window.open(reviewPageUrl, "_blank"); // 새 탭에서 열기
}
